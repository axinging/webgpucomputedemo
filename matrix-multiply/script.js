...
function matrixMultiplyCPU(firstMatrix, secondMatrix) {
  var M = firstMatrix[0];
  var K = firstMatrix[1];
  ...
  result[0] = M;
  result[1] = N;
  for (var m = 0; m < M; m++) {    // Column
    for (var n = 0; n < N; n++) {  // Row
      var acc = 0.0;
      for (var k = 0; k < K; k++) {
        acc += firstMatrix[k * M + m + dataOffset] *
            secondMatrix[n * K + k + dataOffset];
      }
...
async function matrixMultiplyGPU(firstMatrix, secondMatrix, shaderStr) {
  ...
  const shaderModule = device.createShaderModule({code: shaderStr});
